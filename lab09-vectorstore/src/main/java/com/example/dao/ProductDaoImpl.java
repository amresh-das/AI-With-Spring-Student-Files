...
//  TDODO-03: Use a stereotype annotation to mark this DAO as a Spring bean.
...
public class ProductDaoImpl implements ProductDao {
    ...
    //  TDODO-06: Define the public List<String> findClosestMatches() method.
    ...
    public List<String> findClosestMatches(String query,int numberOfMatches) {
        final SearchRequest request = SearchRequest.builder().query(query).topK(numberOfMatches).build();
        final List<Document> documents = vectorStore.similaritySearch(request);
        return documents == null ? List.of() : documents.stream().map(Document::getText).toList();
    }

    //  TDODO-07: Define the public String findClosestMatch() method.
